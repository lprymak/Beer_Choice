        ...
        data.forEach(indx => {
            ...
            if (indx.beer_name == chosenBeers[0] || indx.beer_name == chosenBeers[1] || indx.beer_name == chosenBeers[2]) {
                ...
                if (indx.cluster == i) {
                    ...
                    stats['total_per_cluster'] = indx.cluster_total_reviews_per_beer_per_style;
                    stats['total_overall'] = indx.cluster_total_reviews_per_beer;
                    beers[indx.beer_name] = stats;

                    style_stats['group_overall'] = indx.cluster_review_overall_style;
                    style_stats['group_taste'] = indx.cluster_review_taste_style;
                    style_stats['group_palate'] = indx.cluster_review_palate_style;
                    style_stats['group_aroma'] = indx.cluster_review_aroma_style;
                    style_stats['group_appearance'] = indx.cluster_review_appearance_style;

                    style_stats['mean_overall'] = indx.mean_style_review_overall;
                    style_stats['mean_taste'] = indx.mean_style_review_taste;
                    style_stats['mean_palate'] = indx.mean_style_review_palate;
                    style_stats['mean_aroma'] = indx.mean_style_review_aroma;
                    style_stats['mean_appearance'] = indx.mean_style_review_appearance;
...
function showChoices() {
    ...
    if (newlistedBeers.length == 0) {
        ...
        newlistedBeers.push("No Selection");
    }
    else {
        d3.select('.chosen').classed('vis', true).classed('hid', false);
    }
    console.log(newlistedBeers);
...
function createScatter(data, chosenValue) {
    ...
    var total_reviews = 1580000;
    var total_beers = 65679;
    ...
    console.log(data);
    var total_by_style = [2339, 2910, 2725, 3468, 930, 6004, 14324, 7682, 3108, 4646, 5430, 6019, 4891, 1225];
    var averages = [[3.85, 3.82, 3.82, 3.72, 3.77, 3.96, 3.81, 3.34, 3.90, 3.57, 3.96, 3.90, 3.80, 4.04],
        [3.87, 3.77, 3.83, 3.60, 3.69, 3.97, 3.70, 3.11, 3.90, 3.59, 4.03, 4.02, 3.65, 4.10],
        [3.77, 3.67, 3.79, 3.48, 3.59, 3.95, 3.63, 2.95, 3.84, 3.64, 3.98, 3.97, 3.62, 4.07],
        [3.82, 3.68, 3.77, 3.56, 3.70, 3.91, 3.67, 3.14, 3.79, 3.54, 3.95, 3.94, 3.64, 3.99],
        [3.86, 3.82, 3.93, 3.71, 3.76, 4.00, 3.79, 3.2, 3.96, 3.65, 4.11, 3.99, 3.7, 3.99]];
    ...
    set_styles.forEach((style, i) => {
        ...
        if (data.styles[style]) {
            ...
            value_list['total'] = +data.styles[style].main_style_cluster_count / +data.group_stats.total_beers * 100;
            value_list['all'] = totals[style];
        }
        else {
            value_list['overall'] = 0;
